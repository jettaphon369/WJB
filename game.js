...
try{
  ...
  function render(){
    ...
    renderer.camera.targetY=.7;
    renderer.camera.targetZ=world.player.pos.z;
    renderer.camera.x=world.player.pos.x+1.5;
    renderer.camera.y=8.6;
    renderer.camera.z=world.player.pos.z+11.5;
    ...
    // Player: stylized Seed Ranger
    const p=world.player.pos;
    const f=world.player.facing;
    renderer.drawBox(p.x,.50,p.z,.30,.42,.24,[.16,.50,.26,1],f);
    renderer.drawBox(p.x,.98,p.z,.33,.34,.28,[.24,.64,.34,1],f);
    renderer.drawBox(p.x,1.48,p.z,.25,.25,.25,[.92,.72,.58,1],f);
    renderer.drawBox(
      p.x+Math.sin(f)*.02,
      1.78,
      p.z+Math.cos(f)*.02,
